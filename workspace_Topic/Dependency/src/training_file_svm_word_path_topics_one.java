...
class Topics {
	LinkedList<t_Pairs> class_freq;
	public Topics(LinkedList<t_Pairs> class_freq) {
		this.class_freq = class_freq;
	}
	
	public static t_Pairs max_value(LinkedList<t_Pairs> class_freq) {
		int max_class = -1;
		int max_freq = -1;
		Iterator<t_Pairs> it = class_freq.iterator();
		while (it.hasNext()){
			t_Pairs next_pair = it.next();
			if (next_pair.freq > max_freq) {
				max_class = next_pair.classname;
				max_freq = next_pair.freq;
			}
		}
		
		return new t_Pairs(max_class, max_freq);
	}
	
	public static LinkedList<Integer> one_class(LinkedList<t_Pairs> class_freq1, LinkedList<t_Pairs> class_freq2) {
		LinkedList<Integer> classes = new LinkedList<Integer>();
		for (int i=0 ; i<class_freq1.size(); i++) {
			for (int j=0; j<class_freq2.size(); j++) {
				if (class_freq1.get(i).classname == (class_freq2.get(j).classname)) {
					classes.add(class_freq1.get(i).classname);
				}
			}
		}
...
class t_Pairs {
	int classname;
	int freq;
	public t_Pairs(int classname, int freq) {
		this.classname = classname;
		this.freq = freq;
	}
}

class training_file_svm_word_path_topics_one {
	
	static String folder_path = "/Users/arzookatiyar/Desktop/workspace_Topic/Dependency/Training_data/";
	static String folder_path_svm = "/Users/arzookatiyar/Desktop/workspace_Topic/Dependency/svm_format_word_path_topics_one/";
					...
					if (st.split("\t").length > 2) {
						...
						for (int ind = 2; ind < st.split("\t").length; ind=ind+5) {
							...
							while (it.hasNext()) {
								...
								if (crf_entry.entity_equals(gold)) { //instead of word_path_equals
	...
	public static void training_file_holder_prob(int except) throws IOException{
		...
		for (int fold = 0; fold < 10; fold++) {
			...
			while((st = br.readLine())!=null) {
				...
				//int feature_word_topic = -1;
				LinkedList<Integer> word_path_topic_feature = new LinkedList<Integer>();
				...
				if (word_topic_assignment.
					...
					if (path_topic_assignment.
						...
						word_path_topic_feature = Topics.one_class(word_topic_assignment.
								get(st.split("\t")[1].split("-")[0]).class_freq, path_topic_assignment.
				...
				if (word_path_topic_feature.size() > 0) {
					for (int k=0; k<word_path_topic_feature.size(); k++) {
						output+= "\t"+String.valueOf(word_path_topic_feature.get(k) + 
								features_entity_word1.size() + 
								features_path1.size() + 3)+":"+st.split("\t")[4];
					}
				}
		...
		while((st = br.readLine())!=null) {
			...
			/*if (feature_word_topic!=-1 && Topics.max_value(word_topic_assignment.
					get(st.split("\t")[1].split("-")[0]).class_freq).classname >= 0) {
					output+= "\t"+String.valueOf(feature_word_topic)+":"+st.split("\t")[4];
			}*/
			
			if (st.split("\t").length > 4) {
				/*if (feature_path_topic!=-1 && Topics.max_value(path_topic_assignment.
						get(st.split("\t")[3]).class_freq).classname >= 0) {
						output += "\t"+String.valueOf(feature_path_topic)+":"+st.split("\t")[4];
				}*/
				if (word_path_topic_feature.size() > 0) {
					for (int k=0; k<word_path_topic_feature.size(); k++) {
						output+= "\t"+String.valueOf(word_path_topic_feature.get(k) + 
