...
class final_output_word_path_topics_all_relation {
	
	static String folder_path = "/Users/arzookatiyar/Desktop/workspace_Topic/Dependency/Training_data/";
	static String output_path = "/Users/arzookatiyar/Desktop/workspace_Topic/Dependency/Final_output_word_path_topics_all_relation/";

	static String folder_path_svm = "/Users/arzookatiyar/Desktop/workspace_Topic/Dependency/svm_format_word_path_topics_all_relation/";
	...
	public static int soft_indexOf_relation(LinkedList<Triples2> list, Triples2 entry) {
		Iterator it = list.iterator();
		int index = 0;
		while(it.hasNext()) {
			Triples2 compare = (Triples2)it.next();
			...
			String exp = compare.exp;
			String exp_compare = entry.exp;
			String []exp_words = exp.split(" ");
			String []gold_exp = exp_compare.split(" ");
			...
			if (match_word == true && match_exp == true) {
				return index;
			}
	...
	public static void sentence_triples_relation(String file, String gold_file, int fold, String output_file) throws IOException{
			...
			while(it1.hasNext() && it_prob.hasNext()) {
				...
				if (final_prob > 0) {
					System.out.println(tt.entity_word+"\t"+tt.exp_word+"\t"+tt.path+"\t"
							+tt.entity+"\t"+tt.exp+"\t"
						+final_prob);
					if (soft_indexOf_relation(final_triples, tt) == -1) {
						final_triples.add(tt);
						final_probs.add(final_prob);
					}
					else {
						int index = soft_indexOf_relation(final_triples, tt);
						if (final_probs.get(index) < final_prob) {
							final_triples.set(index, tt);
							final_probs.set(index, final_prob);
						}
	...
	public static void sentence_triples_hard_relation(String file, String gold_file, int fold, String output_file) throws IOException{
	...
	public static void main(String args[]) throws IOException{
		...
		for (int fold = 0; fold<10; fold++) {
			...
			sentence_triples_hard_relation("holders_"+fold, "holders_gold_"+fold, 0, "holders_t_"+fold); //crf_output
		...
		for (int fold = 0; fold<10; fold++) {
			...
			sentence_triples_hard_relation("targets_"+fold, "targets_gold_"+fold, 0, "targets_t_"+fold); //crf_output
		...
		//triples_result("targets_full_t_0", "test_targets_t_0_result"); //file, result_file
		//sentence_triples_hard_relation("targets_0", "targets_gold_0", 0, "targets_t_0"); //crf_output
