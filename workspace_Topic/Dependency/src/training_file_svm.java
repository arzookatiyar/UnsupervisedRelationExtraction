...
class training_file_svm {
	...
	public static void training_file_holder(int except) throws IOException{
		...
		while(it.hasNext()) {
			...
			features_entity_word1.add(feature);
...
class Triples2 {
	...
	public boolean word_equals(Object o) {
		if ((this.entity_word.equals(((Triples2)o).entity_word))) {
			return true;
		}
		return false;
	...
	public boolean entity_equals(Object o) {
		if ((this.entity.equals(((Triples2)o).entity))) {
			return true;
		}
		return false;
	}
	...
	public boolean word_path_equals(Object o) {
		if ((this.entity_word.equals(((Triples2)o).entity_word)) &&
				(this.path.equals(((Triples2)o).path))) {
			return true;
		}
	...
	public boolean word_path_relation_equals(Object o) {
		if ((this.entity_word.equals(((Triples2)o).entity_word)) &&
				(this.exp.equals(((Triples2)o).exp))) {
			return true;
		}
...
/*public static void training_file_holder_bayes(int except) throws IOException{
HashSet<String> features_entity_word = new HashSet<String>();
HashSet<String> features_path = new HashSet<String>();
HashSet<String> features_entity_word_path = new HashSet<String>();
//HashSet<String> features_entity_ = new HashSet<String>();
...
System.out.println(features_entity_word1.size()+"\t"+features_path1.size()+"\t"+
		features_entity_word_path1.size()+"\t"+1);
System.out.println(features_entity_word1.size()+features_path1.size()+features_entity_word_path1.size()+"\t"+1);
System.exit(0);
//features_entity_word1.indexOf(o)
BufferedWriter bw = new BufferedWriter(new FileWriter(folder_path_svm+"training_holders_"+except));
for (int fold = 0; fold < 10; fold++) {
	...
	while((st = br.readLine())!=null) {
		...
		output = st.split("\t")[0];
		//output = output+"\t1:"+ st.split("\t")[1];
		int feature_word = features_entity_word1.indexOf(st.split("\t")[1].split("-")[0]);
		int feature_path = features_path1.indexOf(st.split("\t")[3])+features_entity_word1.size();
		int feature_word_path = features_entity_word_path1.indexOf(st.split("\t")[1].split("-")[0]+
				(st.split("\t")[3])) + features_entity_word1.size() + features_path.size();
		output = output+"\t"+String.valueOf(feature_word)+":1"
		+"\t"+String.valueOf(feature_path)+":1\t"+ String.valueOf(feature_word_path)+":1\n";
		//training_data.add(output);
		bw.write(output);;
