...
class Pair_paths {
	...
	Pair_paths(String holder_path, String target_path) {
...
class other_paths {
	...
	public static void analysis(HashMap<String, Pair_paths> final_100) throws IOException {
		...
		for (String key : final_100.keySet()) {
			...
			for (int i=0; i< words.length; i++) {
				...
				if (!final_100.get(key).holder_word.equals(words[i]) && !(final_100.get(key).target_word.equals(words[i]))) {
					...
					String shortest_path = p.getTheRelationBetween(words[i]+"-"+(i+1), expression_list.get(0));
					for (int k=1; k<expression_list.size(); k++) {
						String path = p.getTheRelationBetween(words[i]+"-"+(i+1), expression_list.get(k));
	...
	public static void all_other_paths() throws IOException{
		...
		while((st = br_t.readLine())!=null) {
			...
			if (holder_map.containsKey(sentence)) {
				...
				common_map.put(sentence, new Pair_paths(holder_map.get(sentence), next_line.split("\t")[2]));
			}
		}
	...
	public static void all_paths_wrt_expression() throws IOException{
...
class Fields {
	...
	public Fields(String expression, String path, String holder_word) {
...
class common_Fields {
	...
	public common_Fields(String expression, String holder_path, String target_path, 
