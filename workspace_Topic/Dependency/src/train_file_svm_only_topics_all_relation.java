...
class train_file_svm_only_topics_all_relation {
	...
	public static void topic_features(String filename) throws IOException{
		...
		while((st = br.readLine())!=null) {
			...
			int total_word_freq = 0;
			for (int i=1; i<word_line.length; i++) {
				total_word_freq += (int)Double.parseDouble(word_line[i].split(",")[1]);
			}
			...
			int total_path_freq = 0;
			for (int i=1; i<path_line.length; i++) {
				total_path_freq += (int)Double.parseDouble(path_line[i].split(",")[1]);
			}
