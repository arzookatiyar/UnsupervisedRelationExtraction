...
class keep_class {
	public static LinkedList<String> create_query_string(int begin, int end, String sentence) {
		...
		/*LinkedList <CoreLabel> query_tokens = new LinkedList<CoreLabel>();
		
		LinkedList <CoreLabel> sentence_tokens = new LinkedList<CoreLabel>();
		...
		loop1 : for (int k=0 ; k<sentence_tokens.size(); k++) {
			...
			another_tradeoff = last_ind - new_begin;
			//System.out.println(another_tradeoff+"\t"+last_ind+"\t"+new_begin);
			if (sentence_tokens.get(k).toString().contains(query_tokens.get(0).toString()) && 
					 tradeoff >= -1 && tradeoff <= 1 && (another_tradeoff >= -3 && another_tradeoff <=3)) {
				//first element matches
				boolean matches = true;
				for (int l=1; l<query_tokens.size(); l++) {
					//System.out.println((k+l)+"\t"+sentence_tokens.get(k+l).toString());
					...
					if (sentence_tokens.get(k+l).toString().contains(query_tokens.get(l).toString())) {
						//do nothing. Good!
					}
					else {
						matches = false;
		...
		if (final_query.size() == 0) {
			...
			//System.out.println(final_query);
			//System.out.println(sentence.substring(begin, end));
			/*query_tokens = new LinkedList<CoreLabel>();

			ptbt = new PTBTokenizer(new StringReader(query_string),
					new CoreLabelTokenFactory(), "");
			for (CoreLabel label; ptbt.hasNext(); ) {
				label = (CoreLabel)ptbt.next();
				//System.out.println(label+"-----");
			...
			ptbt1 = new PTBTokenizer(new StringReader(sentence),
					new CoreLabelTokenFactory(), "");
			for (CoreLabel label1; ptbt1.hasNext(); ) {
				label1 = (CoreLabel)ptbt1.next();
				//System.out.println(label1+"|||");
			...
			query_tokens = new LinkedList<String>(); 
			for (int i=0; i< query_string.split(" ").length; i++) {
				query_tokens.add(query_string.split(" ")[i]);
			}
							
			sentence_tokens = new LinkedList<String>(); 
			for (int j=0; j < sentence.split(" ").length; j++) {
				sentence_tokens.add(sentence.split(" ")[j]);
			}

			
			final_query = new LinkedList<String>();
			//for the two linkedlist find out the overlapping region!
			
			partial_sentence = "";
			
			for (int k=0 ; k<sentence_tokens.size(); k++) {
				partial_sentence = partial_sentence + sentence_tokens.get(k).toString(); 
				//System.out.println(sentence_tokens.get(k));
				//System.out.println(query_tokens.get(0));
				//System.out.println("---"+sentence_tokens.get(k).toString().equals(query_tokens.get(0).toString()));
				...
				int tradeoff = (partial_sentence.indexOf(query_tokens.get(0).toString()) - 
						compare_sentence.indexOf(query_tokens.get(0).toString()));
				if (query_tokens.get(0).toString().equals(sentence_tokens.get(k).toString())) {
					//first element matches
					boolean matches = true;
					//System.out.println("query token size "+query_tokens.size());
					for (int l=1; l<query_tokens.size(); l++) {
						//System.out.println(query_tokens.get(l).toString()+"\t"+sentence_tokens.get(k+l).toString());
						if (query_tokens.get(l).toString().equals(sentence_tokens.get(k+l).toString())) {
							//do nothing. Good!
						}
						else {
							matches = false;
					...
					if (matches == false)
						...
						for (int p=0; p<query_tokens.size(); p++) {
							final_query.add(query_tokens.get(p)+"-"+(k+p+1));
						}
